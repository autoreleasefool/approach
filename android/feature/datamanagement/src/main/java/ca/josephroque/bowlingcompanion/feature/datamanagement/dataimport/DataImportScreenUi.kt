...
sealed interface DataImportScreenUiState {
	...
	data class Loaded(val dataImport: DataImportUiState) : DataImportScreenUiState
}

sealed interface DataImportScreenUiAction {
	data object OnAppear : DataImportScreenUiAction

	data class DataImport(val action: DataImportUiAction) : DataImportScreenUiAction
}
