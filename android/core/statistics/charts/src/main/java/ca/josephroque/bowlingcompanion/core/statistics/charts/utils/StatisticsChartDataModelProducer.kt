...
fun CountableChartData.getModelEntries(): List<ChartEntry> = entries.map { entryOf(it.key.value, it.value.toFloat()) }
fun PercentageChartData.getModelEntries(): List<ChartEntry> = entries.map { entryOf(it.key.value, it.percentage) }
fun AveragingChartData.getModelEntries(): List<ChartEntry> = entries.map { entryOf(it.key.value, it.value) }
