...
class SplitsSparedStatisticTest {
	@Test
	fun testAdjust_ByFramesWithSplitsSpared_Adjusts() {
		val statistic = assertStatisticAdjusts(
			statistic = SplitsSparedStatistic(),
			byFrames = listOf(
				frame(0, listOf(
		...
		assertPercentage(statistic, numerator = 2, denominator = 4, formattedAs = "50% (2)")
	}

	@Test
	fun testAdjust_ByFramesWithoutSplitsSpared_DoesNotAdjust() {
		val statistic = assertStatisticAdjusts(
			statistic = SplitsSparedStatistic(),
			byFrames = listOf(
				frame(0, listOf(
		...
		assertPercentage(statistic, numerator = 0, denominator = 4, formattedAs = "0%", overridingIsEmptyExpectation = true)
	}

	@Test
	fun testAdjust_ByFramesWithSplitsWithBonusSpared_WithBonusEnabled_Adjusts() {
		val statistic = assertStatisticAdjusts(
			statistic = SplitsSparedStatistic(),
			byFrames = listOf(
				frame(0, listOf(
	...
	@Test
	fun testAdjust_ByFramesWithSplitsWithBonusSpared_WithBonusDisabled_DoesNotAdjust() {
		val statistic = assertStatisticAdjusts(
			statistic = SplitsSparedStatistic(),
			byFrames = listOf(
				frame(0, listOf(
	...
	@Test
	fun testAdjust_InLastFrame_ByFramesWithSplitsSpared_Adjusts() {
		val statistic = assertStatisticAdjusts(
			statistic = SplitsSparedStatistic(),
			byFrames = listOf(
				// Open attempt
				frame(Game.NumberOfFrames - 1, listOf(
					roll(0, setOf(Pin.LEFT_THREE_PIN, Pin.HEAD_PIN)),
				...
				frame(Game.NumberOfFrames - 1, listOf(
					...
					roll(2, setOf(Pin.LEFT_TWO_PIN, Pin.RIGHT_THREE_PIN)),
				)),
				...
				frame(Game.NumberOfFrames - 1, listOf(
					...
					roll(2, setOf(Pin.LEFT_THREE_PIN, Pin.HEAD_PIN)),
				)),
				// Three strikes
				frame(Game.NumberOfFrames - 1, listOf(
					roll(0, setOf(Pin.LEFT_TWO_PIN, Pin.LEFT_THREE_PIN, Pin.HEAD_PIN, Pin.RIGHT_THREE_PIN, Pin.RIGHT_TWO_PIN)),
