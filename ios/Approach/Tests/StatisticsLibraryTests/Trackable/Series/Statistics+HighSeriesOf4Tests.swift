...
final class HighSeriesOf4Tests: XCTestCase {
	func testAdjustBy_SeriesWith4Games_Adjusts() {
		let statistic = create(
			statistic: Statistics.HighSeriesOf4.self,
			adjustedBySeries: [
				Series.TrackableEntry(id: UUID(0), numberOfGames: 4, total: 222, date: Date()),
	...
	func testAdjustBy_SeriesNotWith4Games_DoesNotAdjust() {
		let statistic = create(
			statistic: Statistics.HighSeriesOf4.self,
			adjustedBySeries: [
	...
	func testAdjustByGame_DoesNothing() {
		let statistic = create(statistic: Statistics.HighSeriesOf4.self, adjustedByGames: Game.TrackableEntry.mocks)
		AssertHighestOf(statistic, equals: 0)
	}

	func testAdjustByFrame_DoesNothing() {
		let statistic = create(statistic: Statistics.HighSeriesOf4.self, adjustedByFrames: Frame.TrackableEntry.mocks)
		AssertHighestOf(statistic, equals: 0)
	}
}
