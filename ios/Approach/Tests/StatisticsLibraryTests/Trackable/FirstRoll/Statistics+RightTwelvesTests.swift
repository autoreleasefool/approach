...
final class RightTwelvesTests: XCTestCase {
	func testAdjust_ByFramesWithRightTwelves_Adjusts() {
		let statistic = create(
			statistic: Statistics.RightTwelves.self,
			adjustedByFrames: [
	...
	func testAdjust_ByFramesWithoutRightTwelves_DoesNotAdjust() {
		let statistic = create(
			statistic: Statistics.RightTwelves.self,
			adjustedByFrames: [
				Frame.TrackableEntry(
	...
	func testAdjustBySeries_DoesNothing() {
		let statistic = create(statistic: Statistics.RightTwelves.self, adjustedBySeries: Series.TrackableEntry.mocks)
		AssertCounting(statistic, equals: 0)
	}

	func testAdjustByGame_DoesNothing() {
		let statistic = create(statistic: Statistics.RightTwelves.self, adjustedByGames: Game.TrackableEntry.mocks)
		AssertCounting(statistic, equals: 0)
	}
}
