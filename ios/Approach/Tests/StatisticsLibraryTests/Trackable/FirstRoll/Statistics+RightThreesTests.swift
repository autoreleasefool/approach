...
final class RightThreesTests: XCTestCase {
	func testAdjust_ByFramesWithRightThrees_Adjusts() {
		let statistic = create(
			statistic: Statistics.RightThrees.self,
			adjustedByFrames: [
	...
	func testAdjust_ByFramesWithoutRightThrees_DoesNotAdjust() {
		let statistic = create(
			statistic: Statistics.RightThrees.self,
			adjustedByFrames: [
				Frame.TrackableEntry(
	...
	func testAdjustBySeries_DoesNothing() {
		let statistic = create(statistic: Statistics.RightThrees.self, adjustedBySeries: Series.TrackableEntry.mocks)
		AssertCounting(statistic, equals: 0)
	}

	func testAdjustByGame_DoesNothing() {
		let statistic = create(statistic: Statistics.RightThrees.self, adjustedByGames: Game.TrackableEntry.mocks)
		AssertCounting(statistic, equals: 0)
	}
}
