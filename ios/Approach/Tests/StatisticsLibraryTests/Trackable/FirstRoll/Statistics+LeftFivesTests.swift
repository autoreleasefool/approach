...
final class LeftFivesTests: XCTestCase {
	func testAdjust_ByFramesWithLeftFives_Adjusts() {
		let statistic = create(
			statistic: Statistics.LeftFives.self,
			adjustedByFrames: [
	...
	func testAdjust_ByFramesWithoutLeftFives_DoesNotAdjust() {
		let statistic = create(
			statistic: Statistics.LeftFives.self,
			adjustedByFrames: [
				Frame.TrackableEntry(
	...
	func testAdjustBySeries_DoesNothing() {
		let statistic = create(statistic: Statistics.LeftFives.self, adjustedBySeries: Series.TrackableEntry.mocks)
		AssertCounting(statistic, equals: 0)
	}

	func testAdjustByGame_DoesNothing() {
		let statistic = create(statistic: Statistics.LeftFives.self, adjustedByGames: Game.TrackableEntry.mocks)
		AssertCounting(statistic, equals: 0)
	}
}
