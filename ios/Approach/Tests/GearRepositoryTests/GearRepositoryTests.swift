...
final class GearRepositoryTests: XCTestCase {
	...
	func testList_ReturnsAllGear() async throws {
		...
		let db = try await initializeDatabase(withBowlers: .default, withGear: .custom([gear1, gear2]))
	...
	func testCreate_WhenGearExists_ThrowsError() async throws {
		...
		let gear1 = Gear.Database.mock(id: UUID(0), name: "Yellow")
		let db = try await initializeDatabase(withBowlers: .default, withGear: .custom([gear1]))
	...
	func testCreate_WhenGearNotExists_CreatesGear() async throws {
		...
		let db = try await initializeDatabase(withBowlers: .default, withGear: nil)
	...
	func testEdit_WhenGearExists_ReturnsGear() async throws {
		...
		let db = try await initializeDatabase(withBowlers: .default, withGear: .custom([gear]))
