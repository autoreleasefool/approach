...
@Suite("Series+Trackable", .tags(.library))
struct SeriesTrackableTests {

	// MARK: Games

	@Suite("TrackableGames", .tags(.grdb))
	struct TrackableGamesTests {

		@Test("Returns games", .tags(.unit))
		func returnsGames() async throws {
			...
			#expect(result == [games[0]])
		}

		@Test("Games with zero score are not returned", .tags(.unit))
		func gamesWithZeroScoreAreNotReturned() async throws {
			let series = Series.Database.mock(leagueId: UUID(0), id: UUID(0), date: Date(timeIntervalSince1970: 123))
		...
		@Test("Filter by opponent returns games with opponent", .tags(.unit))
		func filterByOpponentReturnsGamesWithOpponent() async throws {
		...
		@Test("Filter by gear returns games with gear", .tags(.unit))
		func filterByGearReturnsGamesWithGear() async throws {
			let bowler = Bowler.Database.mock(id: UUID(0), name: "Joseph")
		...
		@Test("Filter by lanes returns games with lanes", .tags(.unit))
		func filterByLanesReturnsGamesWithLanes() async throws {
			let bowler = Bowler.Database.mock(id: UUID(0), name: "Joseph")
			...
			#expect(result == [game1, game2])
		}

		@Test("Filter by lane positions returns games with lane positions", .tags(.unit))
		func filterByLanePositionsReturnsGames() async throws {
			let bowler = Bowler.Database.mock(id: UUID(0), name: "Joseph")
	...
	@Suite("TrackableFrames", .tags(.grdb))
	struct TrackableFramesTests {

		@Test("Returns expected frames", .tags(.unit))
		func returnsFrames() async throws {
			let series = Series.Database.mock(leagueId: UUID(0), id: UUID(0), date: Date(timeIntervalSince1970: 123))
		...
		@Test("Filtered by gear returns expected frames", .tags(.unit))
		func filteredByGearReturnsFrames() async throws {
			let league = League.Database.mock(id: UUID(0), name: "Majors")
			...
			#expect(result == [frame1, frame2])
		}

		@Test("With all filters returns expected frames", .tags(.unit))
