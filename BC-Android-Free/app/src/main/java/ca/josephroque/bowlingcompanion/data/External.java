...
public class External
{
    ...
    public static Bitmap createImageFromGame(boolean[][][] ballsOfFrame, boolean[][] fouls)
    {
        ...
        for (int frame = Constants.LAST_FRAME; frame >= 0; frame--)
        {
            ...
            {
                if (Arrays.equals(ballsOfFrame[frame][0], Constants.FRAME_PINS_DOWN))
                {
                    canvas.drawText(Constants.BALL_STRIKE, BITMAP_GAME_BALL_WIDTH / 2 + BITMAP_GAME_FRAME_WIDTH * frame, BALL_TEXT_Y, paintText);
                    if (Arrays.equals(ballsOfFrame[frame][1], Constants.FRAME_PINS_DOWN))
                    {
                    ...
                    else
                    {
                        ...
                        if (Arrays.equals(ballsOfFrame[frame][2], Constants.FRAME_PINS_DOWN))
            ...
            if (frame == Constants.LAST_FRAME)
            {
                for (int b = 2; b >= 0; b--)
                {
                    switch(b)
                    {
                        ...
                        case 1:
                        case 0:
                            if (Arrays.equals(ballsOfFrame[frame][b], Constants.FRAME_PINS_DOWN))
                            {
                                frameScores[frame] += GameScore.getValueOfFrame(ballsOfFrame[frame][b]);
                            }
                            break;
            ...
            else
            {
                for (int b = 0; b < 3; b++)
                {
                    if (b < 2 && Arrays.equals(ballsOfFrame[frame][b], Constants.FRAME_PINS_DOWN))
                    {
                        frameScores[frame] += GameScore.getValueOfFrame(ballsOfFrame[frame][b]);
